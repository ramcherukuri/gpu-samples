...
cudaProcess(unsigned int *g_odata, int imgw, int imgh,
            int tilew, int r, float threshold, float highlight, hipTextureObject_t inTex)
{
    // Handle to thread block group
    cg::thread_block cta = cg::this_thread_block();
    HIP_DYNAMIC_SHARED( uchar4, sdata)
                   ...
                   int radius, float threshold, float highlight)
{
    ...
    checkHIPErrors(hipGetChannelDesc(&desc, g_data_array));
    ...
    checkHIPErrors(hipCreateTextureObject(&inTexObject, &texRes, &texDescr, NULL));
    ...
    for (int i = -1; i < nIter; ++i)
    {
        ...
        hipLaunchKernelGGL(cudaProcess, dim3(grid), dim3(block), sbytes , 0, g_odata, imgw, imgh,
                                               block.x+(2*radius), radius, 0.8f, 4.0f, inTexObject);
