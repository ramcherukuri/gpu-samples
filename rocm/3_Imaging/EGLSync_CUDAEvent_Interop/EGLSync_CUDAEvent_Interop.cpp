...
void checkSync(int argc, char **argv)
{
    ...
    device = findHIPDeviceDRV(argc, (const char **)argv);
...
void checkSyncOnCPU(void) 
{
    ...
    rdsc.resType = hipResourceTypeArray;
    ...
    for (z = 0; z < itr; z++)
    {
        ...
        hipLaunchKernelGGL(verify_and_update_kernel, dim3((width*height)/256), dim3(256), 0, 0, writeSurface, readSurface, expectedData, newData, width, height);
...
/*
    ...
    we make use of EGLSync, CUDA Event and hipStreamWaitEvent, eglWaitSyncKHR to achieve the synchronization
...
void checkSyncOnGPU(EGLDisplay dpy) 
{
    ...
    /*
        ...
        3. Using hipStreamWaitEvent() we wait in GPU for the GLES to finish texture copy.
    ...
    */
    for(z = 0; z < itr; z++) {
        ...
        status = cuEventCreateFromEGLSync(&cudaEGLSyncEvent, eglSyncForGL, hipEventDefault);
        if(status != hipSuccess) {
        ...
        hipLaunchKernelGGL(verify_and_update_kernel, dim3((width*height)/256), dim3(256), 0, stream, writeSurface,readSurface, expectedData, newData, width, height);
...
void cleanup(int status)
{
    ...
    if(glErr != GL_NO_ERROR || cudaErr != hipSuccess)
        exitStatus = FAILURE;
