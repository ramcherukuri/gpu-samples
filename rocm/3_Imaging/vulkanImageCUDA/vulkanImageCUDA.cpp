...
class vulkanImageCUDA {
 ...
 private:
  ...
  void initCuda() {
    setCudaVkDevice();
    checkHIPErrors(hipStreamCreate(&streamToRun));
  ...
  void cleanup() {
    ...
    for (int i = 0; i < mipLevels; i++) {
      checkHIPErrors(hipDestroySurfaceObject(surfaceObjectList[i]));
      checkHIPErrors(hipDestroySurfaceObject(surfaceObjectListTemp[i]));
    }

    checkHIPErrors(hipFree(d_surfaceObjectList));
    checkHIPErrors(hipFree(d_surfaceObjectListTemp));
    checkHIPErrors(cudaFreeMipmappedArray(cudaMipmappedImageArrayTemp));
    checkHIPErrors(cudaFreeMipmappedArray(cudaMipmappedImageArrayOrig));
    checkHIPErrors(cudaFreeMipmappedArray(cudaMipmappedImageArray));
    checkHIPErrors(hipDestroyTextureObject(textureObjMipMapInput));
    checkHIPErrors(cudaDestroyExternalMemory(cudaExtMemImageBuffer));
    checkHIPErrors(cudaDestroyExternalSemaphore(cudaExtCudaUpdateVkSemaphore));
    checkHIPErrors(cudaDestroyExternalSemaphore(cudaExtVkUpdateCudaSemaphore));
  ...
  void cudaVkImportSemaphore() {
    ...
    checkHIPErrors(cudaImportExternalSemaphore(&cudaExtCudaUpdateVkSemaphore,
...
#endif
    externalSemaphoreHandleDesc.flags = 0;
    checkHIPErrors(cudaImportExternalSemaphore(&cudaExtVkUpdateCudaSemaphore,
                                                &externalSemaphoreHandleDesc));
  ...
  void cudaVkImportImageMem() {
    ...
    checkHIPErrors(cudaImportExternalMemory(&cudaExtMemImageBuffer,
    ...
    checkHIPErrors(cudaExternalMemoryGetMappedMipmappedArray(
        &cudaMipmappedImageArray, cudaExtMemImageBuffer,
        &externalMemoryMipmappedArrayDesc));

    checkHIPErrors(cudaMallocMipmappedArray(&cudaMipmappedImageArrayTemp,
                                             &formatDesc, extent, mipLevels));
    checkHIPErrors(cudaMallocMipmappedArray(&cudaMipmappedImageArrayOrig,
                                             &formatDesc, extent, mipLevels));
    ...
    for (int mipLevelIdx = 0; mipLevelIdx < mipLevels; mipLevelIdx++) {
      ...
      checkHIPErrors(cudaGetMipmappedArrayLevel(
      ...
      checkHIPErrors(cudaMemcpy2DArrayToArray(
      ...
      checkHIPErrors(hipCreateSurfaceObject(&surfaceObject, &resourceDesc));
    ...
    checkHIPErrors(hipCreateTextureObject(&textureObjMipMapInput, &resDescr,
                                            &texDescr, NULL));

    checkHIPErrors(hipMalloc((void**)&d_surfaceObjectList,
                               sizeof(hipSurfaceObject_t) * mipLevels));
    checkHIPErrors(hipMalloc((void**)&d_surfaceObjectListTemp,
                               sizeof(hipSurfaceObject_t) * mipLevels));

    checkHIPErrors(hipMemcpy(d_surfaceObjectList, surfaceObjectList.data(),
                               sizeof(hipSurfaceObject_t) * mipLevels,
  ...
  void cudaVkSemaphoreSignal(cudaExternalSemaphore_t& extSemaphore) {
    ...
    checkHIPErrors(cudaSignalExternalSemaphoresAsync(
  ...
  void cudaVkSemaphoreWait(cudaExternalSemaphore_t& extSemaphore) {
    ...
    checkHIPErrors(cudaWaitExternalSemaphoresAsync(
