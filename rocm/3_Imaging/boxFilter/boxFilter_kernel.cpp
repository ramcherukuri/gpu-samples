...
__global__ void
d_boxfilter_x_tex(float *od, int w, int h, int r, hipTextureObject_t tex)
{
    float scale = 1.0f / (float)((r << 1) + 1);
    unsigned int y = blockIdx.x*blockDim.x + threadIdx.x;
...
__global__ void
d_boxfilter_y_tex(float *od, int w, int h, int r, hipTextureObject_t tex)
{
    float scale = 1.0f / (float)((r << 1) + 1);
    unsigned int x = blockIdx.x*blockDim.x + threadIdx.x;
...
__global__ void
d_boxfilter_rgba_x(unsigned int *od, int w, int h, int r, hipTextureObject_t rgbaTex)
{
    float scale = 1.0f / (float)((r << 1) + 1);
    unsigned int y = blockIdx.x*blockDim.x + threadIdx.x;
...
void initTexture(int width, int height, void *pImage, bool useRGBA)
{
    ...
    checkHIPErrors(hipMallocArray(&d_array, &channelDesc, width, height));

    size_t bytesPerElem = (useRGBA ? sizeof(uchar4) : sizeof(float));
    checkHIPErrors(hipMemcpy2DToArray(d_array, 0, 0, pImage, width * bytesPerElem, 
                                         width * bytesPerElem, height, hipMemcpyHostToDevice));

    checkHIPErrors(hipMallocArray(&d_tempArray,   &channelDesc, width, height));
    ...
    checkHIPErrors(hipCreateTextureObject(&rgbaTex, &texRes, &texDescr, NULL));
    ...
    checkHIPErrors(hipCreateTextureObject(&rgbaTexTempArray, &texRes, &texDescr, NULL));
    ...
    checkHIPErrors(hipCreateTextureObject(&texTempArray, &texRes, &texDescr, NULL));
}
...
void freeTextures()
{
    ...
    checkHIPErrors(hipDestroyTextureObject(texTempArray));
    checkHIPErrors(hipDestroyTextureObject(rgbaTex));
    checkHIPErrors(hipDestroyTextureObject(rgbaTexTempArray));
    checkHIPErrors(hipFreeArray(d_array));
    checkHIPErrors(hipFreeArray(d_tempArray));
}
                 ...
                 int radius, int iterations, int nthreads, StopWatchInterface *timer)
{
    ...
    for (int i=0; i<iterations; i++)
    {
        ...
        if (iterations > 1){
            hipLaunchKernelGGL(d_boxfilter_x_tex, dim3(height / nthreads), dim3(nthreads), 0 , 0, d_temp, width, height, radius, texTempArray);
        }
        else
        {
            hipLaunchKernelGGL(d_boxfilter_x_tex, dim3(height / nthreads), dim3(nthreads), 0 , 0, d_temp, width, height, radius, tex);
        }

        hipLaunchKernelGGL(d_boxfilter_y_global, dim3(width / nthreads), dim3(nthreads), 0 , 0, d_temp, d_dest, width, height, radius);
        ...
        if (iterations > 1)
        {
            ...
            checkHIPErrors(hipMemcpy2DToArray(d_tempArray, 0, 0, d_dest, width * sizeof(float), 
                     ...
                     int radius, int iterations, int nthreads, StopWatchInterface *timer)
{
    ...
    for (int i=0; i<iterations; i++)
    {
        ...
        if (iterations > 1)
        {
            hipLaunchKernelGGL(d_boxfilter_rgba_x, dim3(height / nthreads), dim3(nthreads), 0 , 0, d_temp, width, height, radius, rgbaTexTempArray);
        }
        else
        {
            hipLaunchKernelGGL(d_boxfilter_rgba_x, dim3(height / nthreads), dim3(nthreads), 0 , 0, d_temp, width, height, radius, rgbaTex);
        }

        hipLaunchKernelGGL(d_boxfilter_rgba_y, dim3(width / nthreads), dim3(nthreads), 0 , 0, d_temp, d_dest, width, height, radius);
        ...
        {
            // copy result back from global memory to array
            checkHIPErrors(hipMemcpy2DToArray(d_tempArray, 0, 0, d_dest, width * sizeof(unsigned int), 
