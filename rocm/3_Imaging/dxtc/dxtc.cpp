...
int main(int argc, char **argv)
{
    ...
    uint *d_data = NULL;
    checkHIPErrors(hipMalloc((void **) &d_data, memSize));

    // Result
    uint *d_result = NULL;
    const uint compressedSize = (w / 4) * (h / 4) * 8;
    checkHIPErrors(hipMalloc((void **)&d_result, compressedSize));
    ...
    checkHIPErrors(hipMalloc((void **) &d_permutations, 1024 * sizeof(uint)));
    checkHIPErrors(hipMemcpy(d_permutations, permutations, 1024 * sizeof(uint),
    ...
    checkHIPErrors(hipMemcpy(d_data, block_image, memSize, hipMemcpyHostToDevice));
    ...
    for (int i = -1; i < numIterations; ++i)
    {
        ...
        {
            hipLaunchKernelGGL(compress, dim3(min(blocksPerLaunch), dim3(blocks-j)), NUM_THREADS, 0, d_permutations, d_data, (uint2 *)d_result, j);
        }
    }

    getLastHIPError("compress");
    ...
    checkHIPErrors(hipMemcpy(h_result, d_result, compressedSize, hipMemcpyDeviceToHost));
    ...
    checkHIPErrors(hipFree(d_permutations));
    checkHIPErrors(hipFree(d_data));
    checkHIPErrors(hipFree(d_result));
    free(image_path);
    free(data);
    free(block_image);
    free(h_result);
