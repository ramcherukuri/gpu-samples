...
runTest(int argc, char **argv)
{
    ...
    checkHIPErrors(hipMalloc((void **) &d_idata, smem_size));
    checkHIPErrors(hipMalloc((void **) &d_odata, smem_size));
    checkHIPErrors(hipMalloc((void **) &approx_final, smem_size));
    // copy input data to device
    checkHIPErrors(hipMemcpy(d_idata, signal, smem_size,
    ...
    hipLaunchKernelGGL(initValue, dim3(grid_size), dim3(block_size), 0, 0, d_odata, 0.0f);
    ...
    while (0 != num_threads_total_left)
    {
        ...
        if (approx_is_input)
        {
            checkHIPErrors(hipMemcpy(d_idata, approx_final, grid_size.x * 4,
                                       hipMemcpyDeviceToDevice));
        }
        else
        {
            checkHIPErrors(hipMemcpy(d_odata, approx_final, grid_size.x * 4,
                                       hipMemcpyDeviceToDevice));
        }
    ...
    checkHIPErrors(hipMemcpy(odata, d_odata, smem_size,
    ...
    checkHIPErrors(hipFree(approx_final));
