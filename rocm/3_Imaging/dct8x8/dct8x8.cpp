...
float WrapperCUDA1(byte *ImgSrc, byte *ImgDst, int Stride, ROI Size)
{
    ...
    float *Dst;
    size_t DstStride;
    checkHIPErrors(hipMallocArray(&Src, &floattex, Size.width, Size.height));
    checkHIPErrors(hipMallocPitch((void **)(&Dst), &DstStride, Size.width * sizeof(float), Size.height));
    ...
    checkHIPErrors(hipMemcpy2DToArray(Src, 0, 0,
    ...
    checkHIPErrors(hipCreateTextureObject(&TexSrc, &texRes, &texDescr, NULL));

    for (int i=0; i<BENCHMARK_SIZE; i++)
    {
        sdkStartTimer(&timerCUDA);
        hipLaunchKernelGGL(CUDAkernel1DCT, dim3(grid), dim3(threads ), 0, 0, Dst, (int) DstStride, 0, 0, TexSrc);
    ...
    // execute IDCT kernel
    hipLaunchKernelGGL(CUDAkernel1IDCT, dim3(grid), dim3(threads ), 0, 0, Dst, (int) DstStride, 0, 0, TexSrc);
    getLastHIPError("Kernel execution failed");

    //copy quantized image block to host
    checkHIPErrors(hipMemcpy2D(ImgSrcF, ImgSrcFStride *sizeof(float),
                                 Dst, DstStride *sizeof(float),
                                 Size.width *sizeof(float), Size.height,
    ...
    //clean up memory
    checkHIPErrors(hipDestroyTextureObject(TexSrc));
    checkHIPErrors(hipFreeArray(Src));
    checkHIPErrors(hipFree(Dst));
    FreePlane(ImgSrcF);
...
float WrapperCUDA2(byte *ImgSrc, byte *ImgDst, int Stride, ROI Size)
{
    ...
    float *src, *dst;
    size_t DeviceStride;
    checkHIPErrors(hipMallocPitch((void **)&src, &DeviceStride, Size.width * sizeof(float), Size.height));
    checkHIPErrors(hipMallocPitch((void **)&dst, &DeviceStride, Size.width * sizeof(float), Size.height));
    DeviceStride /= sizeof(float);

    //copy from host memory to device
    checkHIPErrors(hipMemcpy2D(src, DeviceStride * sizeof(float),
    ...
    for (int i = -1; i < numIterations; i++)
    {
        ...
        hipLaunchKernelGGL(CUDAkernel2DCT, dim3(GridFullWarps), dim3(ThreadsFullWarps), 0, 0, dst, src, (int)DeviceStride);
        getLastHIPError("Kernel execution failed");
    }
    ...
    // execute Quantization kernel
    hipLaunchKernelGGL(CUDAkernelQuantizationFloat, dim3(GridSmallBlocks), dim3(ThreadsSmallBlocks ), 0, 0, dst, (int) DeviceStride);
    getLastHIPError("Kernel execution failed");

    //perform block-wise IDCT processing
    hipLaunchKernelGGL(CUDAkernel2IDCT, dim3(GridFullWarps), dim3(ThreadsFullWarps ), 0, 0, src, dst, (int)DeviceStride);
    checkHIPErrors(hipDeviceSynchronize());
    getLastHIPError("Kernel execution failed");

    //copy quantized image block to host
    checkHIPErrors(hipMemcpy2D(ImgF1, StrideF *sizeof(float),
...
float WrapperCUDAshort(byte *ImgSrc, byte *ImgDst, int Stride, ROI Size)
{
    ...
    short *SrcDst;
    size_t DeviceStride;
    checkHIPErrors(hipMallocPitch((void **)(&SrcDst), &DeviceStride, Size.width * sizeof(short), Size.height));
    DeviceStride /= sizeof(short);

    //copy from host memory to device
    checkHIPErrors(hipMemcpy2D(SrcDst, DeviceStride * sizeof(short),
    ...
    //copy quantized image block to host
    checkHIPErrors(hipMemcpy2D(ImgS1, StrideS *sizeof(short),
