...
void resizeNV12Batch(uint8_t *dpSrc, int nSrcPitch, int nSrcWidth,
  ...
  checkHIPErrors(hipCreateTextureObject(&texLuma, &resDesc, &texDesc, NULL));

  resDesc.res.pitch2D.desc = hipCreateChannelDesc<uchar2>();
  resDesc.res.pitch2D.width /= 2;

  hipTextureObject_t texChroma = 0;
  checkHIPErrors(hipCreateTextureObject(&texChroma, &resDesc, &texDesc, NULL));
  ...
  checkHIPErrors(hipDestroyTextureObject(texLuma));
  checkHIPErrors(hipDestroyTextureObject(texChroma));
}
