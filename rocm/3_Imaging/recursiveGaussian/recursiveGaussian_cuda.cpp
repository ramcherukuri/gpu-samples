...
void gaussianFilterRGBA(uint *d_src, uint *d_dest, uint *d_temp, int width, int height, float sigma, int order, int nthreads)
{
    ...
    hipLaunchKernelGGL(d_simpleRecursive_rgba, dim3(iDivUp(width), dim3(nthreads)), nthreads , 0, d_src, d_temp, width, height, ema);
#else
    hipLaunchKernelGGL(d_recursiveGaussian_rgba, dim3(iDivUp(width), dim3(nthreads)), nthreads , 0, d_src, d_temp, width, height, a0, a1, a2, a3, b1, b2, coefp, coefn);
#endif
    getLastHIPError("Kernel execution failed");

    transpose(d_temp, d_dest, width, height);
    getLastHIPError("transpose: Kernel execution failed");

    // process rows
#if USE_SIMPLE_FILTER
    hipLaunchKernelGGL(d_simpleRecursive_rgba, dim3(iDivUp(height), dim3(nthreads)), nthreads , 0, d_dest, d_temp, height, width, ema);
#else
    hipLaunchKernelGGL(d_recursiveGaussian_rgba, dim3(iDivUp(height), dim3(nthreads)), nthreads , 0, d_dest, d_temp, height, width, a0, a1, a2, a3, b1, b2, coefp, coefn);
#endif
    getLastHIPError("Kernel execution failed");
