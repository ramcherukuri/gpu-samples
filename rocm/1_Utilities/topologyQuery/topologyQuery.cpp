...
int main(int argc, char **argv)
{
    ...
    for (int device1 = 0; device1 < deviceCount; device1++)
    {
        for (int device2 = 0; device2 < deviceCount; device2++)
        {
            ...
            checkHIPErrors(cudaDeviceGetP2PAttribute(&accessSupported, cudaDevP2PAttrAccessSupported, device1, device2));
            checkHIPErrors(cudaDeviceGetP2PAttribute(&perfRank, cudaDevP2PAttrPerformanceRank, device1, device2));
            checkHIPErrors(cudaDeviceGetP2PAttribute(&atomicSupported, cudaDevP2PAttrNativeAtomicSupported, device1, device2));
    ...
    {
        int atomicSupported = 0;
        checkHIPErrors(hipDeviceGetAttribute(&atomicSupported, cudaDevAttrHostNativeAtomicSupported, device));
