...
void performP2PCopy(int *dest, int destDevice, int *src, int srcDevice, int num_elems, int repeat, bool p2paccess, hipStream_t streamToRun)
{
    int blockSize = 0;
    int numBlocks = 0;
    ...
    if (p2p_mechanism == SM && p2paccess)
    {
        for (int r = 0; r < repeat; r++) {
            hipLaunchKernelGGL(copyp2p, dim3(numBlocks), dim3(blockSize), 0, streamToRun, (int4*)dest, (int4*)src, num_elems/4);
        }
    }
    else
    {
        for (int r = 0; r < repeat; r++) {
            hipMemcpyPeerAsync(dest, destDevice, src, srcDevice, sizeof(int)*num_elems, streamToRun);
        }
    }
}
