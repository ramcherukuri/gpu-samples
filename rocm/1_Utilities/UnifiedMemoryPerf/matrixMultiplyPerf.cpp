...
void runMatrixMultiplyKernel(unsigned int matrixDim, int allocType,
  ...
  checkHIPErrors(hipGetDeviceProperties(&deviceProp, device_id));
  checkHIPErrors(hipStreamCreate(&streamToRunOn));
  ...
  checkHIPErrors(hipMalloc(&dptrA, size));
  checkHIPErrors(hipMalloc(&dptrB, size));
  checkHIPErrors(hipMalloc(&dptrC, size));
  ...
  checkHIPErrors(hipMemcpyAsync(randValuesVerifyXmulY, dptrC, size,
                                  hipMemcpyDeviceToHost));
  checkHIPErrors(hipStreamSynchronize(NULL));
  hipLaunchKernelGGL(matrixMultiplyKernel, dim3(grid), dim3(threads), 0, 0, dptrC, dptrB, dptrA, matrixDim);
  checkHIPErrors(hipMemcpyAsync(randValuesVerifyYmulX, dptrC, size,
                                  hipMemcpyDeviceToHost));
  checkHIPErrors(hipStreamSynchronize(NULL));
#if VERIFY_GPU_CORRECTNESS
...
#endif
  checkHIPErrors(hipFree(dptrA));
  checkHIPErrors(hipFree(dptrB));
  checkHIPErrors(hipFree(dptrC));

  checkHIPErrors(hipHostMalloc(&latch, sizeof(unsigned int)));
  ...
  switch (allocType) {
    ...
    case USE_HOST_PAGELOCKED_AND_DEVICE_MEMORY_ASYNC:
      checkHIPErrors(hipHostMalloc(&hptrA, size));
      checkHIPErrors(hipHostMalloc(&hptrB, size));
      checkHIPErrors(hipHostMalloc(&hptrC, size));
      checkHIPErrors(hipMalloc(&dptrA, size));
      checkHIPErrors(hipMalloc(&dptrB, size));
      checkHIPErrors(hipMalloc(&dptrC, size));
      copyRequired = true;
    ...
    case USE_ZERO_COPY:
      ...
      checkHIPErrors(hipHostMalloc(&hptrB, size));
      checkHIPErrors(hipHostMalloc(&hptrC, size));
      checkHIPErrors(hipHostGetDevicePointer(&dptrA, hptrA, 0));
      checkHIPErrors(hipHostGetDevicePointer(&dptrB, hptrB, 0));
      checkHIPErrors(hipHostGetDevicePointer(&dptrC, hptrC, 0));
      break;

    case USE_MANAGED_MEMORY:
      checkHIPErrors(hipMallocManaged(&dptrA, size));
      checkHIPErrors(hipMallocManaged(&dptrB, size));
      checkHIPErrors(hipMallocManaged(&dptrC, size));
      hptrA = dptrA;
      hptrB = dptrB;
      hptrC = dptrC;
      break;
    ...
    case USE_MANAGED_MEMORY_WITH_HINTS_ASYNC:
      if (deviceProp.concurrentManagedAccess) {
        ...
        checkHIPErrors(hipMallocManaged(&dptrB, size));
        checkHIPErrors(hipMallocManaged(&dptrC, size));
        checkHIPErrors(cudaMemPrefetchAsync(dptrA, size, cudaCpuDeviceId));
        checkHIPErrors(cudaMemPrefetchAsync(dptrB, size, cudaCpuDeviceId));
        checkHIPErrors(cudaMemPrefetchAsync(dptrC, size, cudaCpuDeviceId));
      } else {
        checkHIPErrors(hipMallocManaged(&dptrA, size, hipMemAttachHost));
        checkHIPErrors(hipMallocManaged(&dptrB, size, hipMemAttachHost));
        checkHIPErrors(hipMallocManaged(&dptrC, size, hipMemAttachHost));
      }
      hptrA = dptrA;
      hptrB = dptrB;
      hptrC = dptrC;
    ...
    if (someTransferOpRequired) {
      ...
      if (hintsRequired) {
        if (deviceProp.concurrentManagedAccess) {
          ...
          checkHIPErrors(cudaStreamAttachMemAsync(streamToRunOn, dptrA, 0,
                                                   hipMemAttachGlobal));
          checkHIPErrors(cudaStreamAttachMemAsync(streamToRunOn, dptrB, 0,
                                                   hipMemAttachGlobal));
          checkHIPErrors(cudaStreamAttachMemAsync(streamToRunOn, dptrC, 0,
                                                   hipMemAttachGlobal));
        }
        if (!isAsync) {
          checkHIPErrors(hipStreamSynchronize(streamToRunOn));
        }
      }
  ...
  switch (allocType) {
    ...
    case USE_HOST_PAGELOCKED_AND_DEVICE_MEMORY:
    case USE_HOST_PAGELOCKED_AND_DEVICE_MEMORY_ASYNC:
      checkHIPErrors(hipHostFree(hptrA));
      checkHIPErrors(hipHostFree(hptrB));
      checkHIPErrors(hipHostFree(hptrC));
      checkHIPErrors(hipFree(dptrA));
      checkHIPErrors(hipFree(dptrB));
      checkHIPErrors(hipFree(dptrC));
      break;
  ...
  checkHIPErrors(hipStreamDestroy(streamToRunOn));
  checkHIPErrors(hipHostFree(latch));
  free(randValuesX);
  free(randValuesY);
...
int main(int argc, char **argv) {
  ...
  int device_id = findHIPDevice(argc, (const char **)argv);
