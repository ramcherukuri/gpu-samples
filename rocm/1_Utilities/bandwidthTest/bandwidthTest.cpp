...
testDeviceToHostTransfer(unsigned int memSize, memoryMode memMode, bool wc)
{
...
#if CUDART_VERSION >= 2020
        checkHIPErrors(hipHostMalloc((void **)&h_idata, memSize, (wc) ? hipHostMallocWriteCombined : 0));
        checkHIPErrors(hipHostMalloc((void **)&h_odata, memSize, (wc) ? hipHostMallocWriteCombined : 0));
#else
    ...
    checkHIPErrors(hipMemcpy(d_idata, h_idata, memSize,
    ...
    if (PINNED == memMode)
    {
        ...
        {
            checkHIPErrors(hipMemcpyAsync(h_odata, d_idata, memSize,
    ...
    else
    {
        for (unsigned int i = 0; i < MEMCOPY_ITERATIONS; i++)
        {
            checkHIPErrors(hipMemcpy(h_odata, d_idata, memSize,
...
testHostToDeviceTransfer(unsigned int memSize, memoryMode memMode, bool wc)
{
    ...
    if (PINNED == memMode)
    {
        for (unsigned int i = 0; i < MEMCOPY_ITERATIONS; i++)
        {
            checkHIPErrors(hipMemcpyAsync(d_idata, h_odata, memSize,
    ...
    else
    {
        for (unsigned int i = 0; i < MEMCOPY_ITERATIONS; i++)
        {
            checkHIPErrors(hipMemcpy(d_idata, h_odata, memSize,
                                       hipMemcpyHostToDevice));
        }
    }
...
testDeviceToDeviceTransfer(unsigned int memSize)
{
    ...
    for (unsigned int i = 0; i < MEMCOPY_ITERATIONS; i++)
    {
        checkHIPErrors(hipMemcpy(d_odata, d_idata, memSize,
