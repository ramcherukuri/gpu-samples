...
int MatrixMultiply(int argc, char **argv,
    ...
    if (block_size == 16) {
        hipLaunchKernelGGL(HIP_KERNEL_NAME(MatrixMulCUDA<16>), dim3(grid), dim3(threads), 0, stream, d_C, d_A, d_B,
                                                dimsA.x, dimsB.x);
    } else {
        hipLaunchKernelGGL(HIP_KERNEL_NAME(MatrixMulCUDA<32>), dim3(grid), dim3(threads), 0, stream, d_C, d_A, d_B,
                                                dimsA.x, dimsB.x);
    }
