...
CUT_THREADPROC launch(void *void_arg)
{
    ...
    checkHIPErrors(hipSetDevice(workload->cudaDeviceID));

    // Allocate Resources
    checkHIPErrors(hipStreamCreate(&workload->stream));
    checkHIPErrors(hipMalloc(&workload->d_data, N_elements_per_workload * sizeof(int)));
    checkHIPErrors(hipHostMalloc(&workload->h_data, N_elements_per_workload * sizeof(int), hipHostMallocPortable));
    ...
    checkHIPErrors(hipMemcpyAsync(workload->d_data, workload->h_data, N_elements_per_workload * sizeof(int), hipMemcpyHostToDevice, workload->stream));
    hipLaunchKernelGGL(incKernel, dim3(grid), dim3(block), 0, workload->stream, workload->d_data, N_elements_per_workload);
    checkHIPErrors(hipMemcpyAsync(workload->h_data, workload->d_data, N_elements_per_workload * sizeof(int), hipMemcpyDeviceToHost, workload->stream));
    ...
    checkHIPErrors(hipStreamAddCallback(workload->stream, myStreamCallback, workload, 0));
...
CUT_THREADPROC postprocess(void *void_arg)
{
    ...
    // Free Resources
    checkHIPErrors(hipFree(workload->d_data));
    checkHIPErrors(hipHostFree(workload->h_data));
    checkHIPErrors(hipStreamDestroy(workload->stream));
...
int main(int argc, char **argv)
{
    ...
    checkHIPErrors(hipGetDeviceCount(&N_gpus));
