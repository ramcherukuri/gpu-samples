...
runTest(const int argc, const char **argv, char *data, int2 *data_int2, unsigned int len)
{
    ...
    char *d_data;
    checkHIPErrors(hipMalloc((void **) &d_data, mem_size));
    // copy host memory to device
    checkHIPErrors(hipMemcpy(d_data, data, mem_size,
                               hipMemcpyHostToDevice));
    // allocate device memory for int2 version
    int2 *d_data_int2;
    checkHIPErrors(hipMalloc((void **) &d_data_int2, mem_size_int2));
    // copy host memory to device
    checkHIPErrors(hipMemcpy(d_data_int2, data_int2, mem_size_int2,
                               hipMemcpyHostToDevice));
    ...
    checkHIPErrors(hipMemcpy(data, d_data, mem_size,
                               hipMemcpyDeviceToHost));
    checkHIPErrors(hipMemcpy(data_int2, d_data_int2, mem_size_int2,
                               hipMemcpyDeviceToHost));
    ...
    checkHIPErrors(hipFree(d_data_int2));
    free(reference);
    free(reference2);
