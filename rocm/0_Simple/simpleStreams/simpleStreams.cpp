...
AllocateHostMemory(bool bPinGenericMemory, int **pp_a, int **ppAligned_a, int nbytes)
{
    ...
    if (bPinGenericMemory)
    {
        ...
        *ppAligned_a = (int *)ALIGN_UP(*pp_a, MEMORY_ALIGNMENT);
        ...
        // pin allocate memory
        checkHIPErrors(hipHostRegister(*ppAligned_a, nbytes, hipHostRegisterMapped));
    }
    else
#endif
#endif
    {
        ...
        checkHIPErrors(hipHostMalloc((void **)pp_a, nbytes));
        *ppAligned_a = *pp_a;
    }
}
...
FreeHostMemory(bool bPinGenericMemory, int **pp_a, int **ppAligned_a, int nbytes)
{
    ...
    if (bPinGenericMemory)
    {
        ...
        checkHIPErrors(hipHostUnregister(*ppAligned_a));
#ifdef WIN32
...
int main(int argc, char **argv)
{
    ...
    checkHIPErrors(hipSetDevice(cuda_device));
    ...
    checkHIPErrors(hipSetDeviceFlags(device_sync_method | (bPinGenericMemory ? hipDeviceMapHost : 0)));
    ...
    checkHIPErrors(hipMemset(d_a, 0x0, nbytes));
    checkHIPErrors(hipMalloc((void **)&d_c, sizeof(int)));
    checkHIPErrors(hipMemcpy(d_c, &c, sizeof(int), hipMemcpyHostToDevice));
    ...
    checkHIPErrors(hipEventRecord(start_event, 0));     // record in stream-0, to ensure that all previous CUDA calls have completed
    checkHIPErrors(hipMemcpyAsync(hAligned_a, d_a, nbytes, hipMemcpyDeviceToHost, streams[0]));
    checkHIPErrors(hipEventRecord(stop_event, 0));
    checkHIPErrors(hipEventSynchronize(stop_event));   // block until the event is actually recorded
    ...
    checkHIPErrors(hipEventElapsedTime(&time_kernel, start_event, stop_event));
    printf("kernel:\t\t%.2f\n", time_kernel);
    ...
    for (int k = 0; k < nreps; k++)
    {
        ...
        checkHIPErrors(hipMemcpy(hAligned_a, d_a, nbytes, hipMemcpyDeviceToHost));
    }
    ...
    checkHIPErrors(hipMemset(d_a, 0, nbytes)); // set device memory to all 0s, for testing correctness
    checkHIPErrors(hipEventRecord(start_event, 0));
    ...
    for (int k = 0; k < nreps; k++)
    {
        ...
        for (int i = 0; i < nstreams; i++)
        {
            checkHIPErrors(hipMemcpyAsync(hAligned_a + i * n / nstreams, d_a + i * n / nstreams, nbytes / nstreams, hipMemcpyDeviceToHost, streams[i]));
        }
    }
