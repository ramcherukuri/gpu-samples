...
void launchGrayScaleKernel(unsigned int* d_rgbaImage, std::string image_filename,
                            size_t imageWidth, size_t imageHeight, hipStream_t stream)
{
    int numThreadsPerBlock = 1024;
    int numOfBlocks = (imageWidth*imageHeight) / numThreadsPerBlock;

    hipLaunchKernelGGL(rgbToGrayscaleKernel, dim3(numOfBlocks), dim3(numThreadsPerBlock), 0, stream, d_rgbaImage, imageWidth, imageHeight);

    unsigned int *outputData = (unsigned int*)malloc(sizeof(unsigned int)*imageWidth*imageHeight);
    checkHIPErrors(hipMemcpyAsync(outputData, d_rgbaImage, sizeof(unsigned int)*imageWidth*imageHeight, hipMemcpyDeviceToHost, stream));
...
void rotateKernel(hipTextureObject_t &texObj, const float angle, unsigned int *d_outputData, const int imageWidth, const int imageHeight, hipStream_t stream)
{
    dim3 dimBlock(8, 8, 1);
    dim3 dimGrid(imageWidth / dimBlock.x, imageHeight / dimBlock.y, 1);

    hipLaunchKernelGGL(transformKernel, dim3(dimGrid), dim3(dimBlock), 0, stream, d_outputData, imageWidth, imageHeight, angle, texObj);
}
