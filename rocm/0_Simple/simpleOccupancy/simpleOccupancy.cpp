...
{
    HIP_DYNAMIC_SHARED( int, dynamicSmem)
...
static int launchConfig(int *array, int arrayCount, bool automatic)
{
    ...
    if (automatic) {
        checkHIPErrors(hipOccupancyMaxPotentialBlockSize(
                            &minGridSize,
                            &blockSize,
                            (void*)square,
                            dynamicSMemUsage,
    ...
    // Report elapsed time
    //
    checkHIPErrors(hipEventElapsedTime(&elapsedTime, start, end));
...
static int test(bool automaticLaunchConfig, const int count = 1000000)
{
    ...
    checkHIPErrors(hipMalloc(&dArray, size));
    checkHIPErrors(hipMemcpy(dArray, array, size, hipMemcpyHostToDevice));
    ...
    checkHIPErrors(hipMemcpy(array, dArray, size, hipMemcpyDeviceToHost));
    checkHIPErrors(hipFree(dArray));
