...
int main(int argc, char **argv)
{
    ...
    checkHIPErrors(hipMalloc((void**)&A, sizeof(uint8_t) * M_GLOBAL * K_GLOBAL));
    checkHIPErrors(hipMalloc((void**)&B, sizeof(uint8_t) * N_GLOBAL * K_GLOBAL));
    checkHIPErrors(hipMalloc((void**)&C, sizeof(int) * M_GLOBAL * N_GLOBAL));
    checkHIPErrors(hipMalloc((void**)&D, sizeof(int) * M_GLOBAL * N_GLOBAL));
    ...
    checkHIPErrors(hipMemcpy(A, A_h, sizeof(uint8_t) * M_GLOBAL * K_GLOBAL, hipMemcpyHostToDevice));
    checkHIPErrors(hipMemcpy(B, B_h, sizeof(uint8_t) * N_GLOBAL * K_GLOBAL, hipMemcpyHostToDevice));
    checkHIPErrors(hipMemcpy(C, C_h, sizeof(int) * M_GLOBAL * N_GLOBAL, hipMemcpyHostToDevice));
    checkHIPErrors(hipMemset(D, 0,   sizeof(int) * M_GLOBAL * N_GLOBAL));
    ...
    if (deviceProp.sharedMemPerMultiprocessor >= SHMEM_SZ)
    {
        ...
        checkHIPErrors(cudaFuncSetAttribute(compute_gemm_imma, cudaFuncAttributeMaxDynamicSharedMemorySize, SHMEM_SZ));
        checkKernelErrors((hipLaunchKernelGGL(compute_gemm_imma, dim3(deviceProp.multiProcessorCount), dim3(THREADS_PER_BLOCK), SHMEM_SZ, 0, A, B, C, D, alpha, beta)));
#if CPU_DEBUG
        checkHIPErrors(hipMemcpy(result_hD, D, sizeof(int)*M_GLOBAL*N_GLOBAL, hipMemcpyDeviceToHost));
#endif
    }
    else
    {
        ...
        hipLaunchKernelGGL(simple_wmma_gemm_imma, dim3(gridDim), dim3(blockDim), 0, 0, A, B, C, D, M_GLOBAL, N_GLOBAL, K_GLOBAL, alpha, beta);
#if CPU_DEBUG
        checkHIPErrors(hipMemcpy(result_hD, D, sizeof(int) * M_GLOBAL * N_GLOBAL, hipMemcpyDeviceToHost));
#endif
    }
    ...
    checkHIPErrors(hipEventElapsedTime(&milliseconds, start, stop));
    ...
    free(B_h);
    free(C_h);
    checkHIPErrors(hipFree((void*)A));
    checkHIPErrors(hipFree((void*)B));
    checkHIPErrors(hipFree((void*)C));
    checkHIPErrors(hipFree((void*)D));
