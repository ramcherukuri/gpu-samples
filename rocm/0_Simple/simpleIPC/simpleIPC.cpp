...
typedef struct shmStruct_st {
    ...
    ihipIpcEventHandle_t eventHandle[MAX_DEVICES];
} shmStruct;
...
static void childProcess(int id)
{
    ...
    checkHIPErrors(hipSetDevice(shm->devices[id]));
    checkHIPErrors(hipGetDeviceProperties(&prop, shm->devices[id]));
    checkHIPErrors(hipStreamCreateWithFlags(&stream, hipStreamNonBlocking));
    checkHIPErrors(hipOccupancyMaxActiveBlocksPerMultiprocessor(&blocks, simpleKernel, threads, 0));
    ...
    for (i = 0; i < procCount; i++) {
        ...
        checkHIPErrors(hipIpcOpenMemHandle(&ptr, *(hipIpcMemHandle_t*)&shm->memHandle[i], hipIpcMemLazyEnablePeerAccess));
        checkHIPErrors(hipIpcOpenEventHandle(&event, *(ihipIpcEventHandle_t*)&shm->eventHandle[i]));
    ...
    for (i = 0; i < procCount; i++) {
        ...
        checkHIPErrors(hipStreamWaitEvent(stream, events[bufferId], 0));
        // Push a simple kernel on it
        hipLaunchKernelGGL(simpleKernel, dim3(blocks), dim3(threads), 0, stream, (char *)ptrs[bufferId], DATA_SIZE, id);
        checkHIPErrors(hipGetLastError());
        // Signal that this buffer is ready for the next consumer
        checkHIPErrors(hipEventRecord(events[bufferId], stream));
    ...
    checkHIPErrors(hipStreamWaitEvent(stream, events[id], 0));
    checkHIPErrors(hipMemcpyAsync(&verification_buffer[0], ptrs[id], DATA_SIZE, hipMemcpyDeviceToHost, stream));
    ...
    // Clean up!
    for (i = 0; i < procCount; i++) { 
        checkHIPErrors(hipIpcCloseMemHandle(ptrs[i]));
        checkHIPErrors(hipEventDestroy(events[i]));
    }
...
static void parentProcess(char *app)
{
    ...
    checkHIPErrors(hipGetDeviceCount(&devCount));
    ...
    for (i = 0; i < devCount; i++) {
        bool allPeers = true;
        hipDeviceProp_t prop;
        checkHIPErrors(hipGetDeviceProperties(&prop, i));
        ...
        for (int j = 0; j < shm->nprocesses; j++) {
            ...
            checkHIPErrors(hipDeviceCanAccessPeer(&canAccessPeerJI, shm->devices[j], i));
            checkHIPErrors(hipDeviceCanAccessPeer(&canAccessPeerIJ, i, shm->devices[j]));
        ...
        if (allPeers) {
            ...
            for (int j = 0; j < shm->nprocesses; j++) {
                ...
                checkHIPErrors(hipDeviceEnablePeerAccess(shm->devices[j], 0));
                checkHIPErrors(hipSetDevice(shm->devices[j]));
                checkHIPErrors(hipDeviceEnablePeerAccess(i, 0));
            }
    ...
    for (i = 0; i < shm->nprocesses; i++) {
        ...
        checkHIPErrors(hipSetDevice(shm->devices[i]));
        checkHIPErrors(hipMalloc(&ptr, DATA_SIZE));
        checkHIPErrors(hipIpcGetMemHandle((hipIpcMemHandle_t *)&shm->memHandle[i], ptr));
        checkHIPErrors(hipEventCreate(&event, hipEventDisableTiming | hipEventInterprocess));
        checkHIPErrors(hipIpcGetEventHandle((ihipIpcEventHandle_t *)&shm->eventHandle[i], event));
    ...
    for (i = 0; i < shm->nprocesses; i++) {
        checkHIPErrors(hipSetDevice(shm->devices[i]));
        checkHIPErrors(hipEventSynchronize(events[i]));
        checkHIPErrors(hipEventDestroy(events[i]));
        checkHIPErrors(hipFree(ptrs[i]));
    }
