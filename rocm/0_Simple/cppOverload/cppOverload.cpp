...
int main(int argc, const char *argv[])
{
    ...
    printf("DevicecheckHIPErrors Count: %d\n", deviceCount);

    int deviceID = findHIPDevice(argc, argv);
    ...
    checkHIPErrors(hipMalloc(&dInput , sizeof(int)*N*2));
    checkHIPErrors(hipMalloc(&dOutput, sizeof(int)*N));

    // Allocate host memory
    checkHIPErrors(hipHostMalloc(&hInput , sizeof(int)*N*2));
    checkHIPErrors(hipHostMalloc(&hOutput, sizeof(int)*N));
    ...
    checkHIPErrors(hipMemcpy(dInput, hInput, sizeof(int)*N*2, hipMemcpyHostToDevice));
    ...
    checkHIPErrors(hipFuncSetCacheConfig(reinterpret_cast<const void*>(*func1), hipFuncCachePreferShared));
    checkHIPErrors(hipFuncGetAttributes(&attr, reinterpret_cast<const void*>(*func1)));
    OUTPUT_ATTR(attr);
    (*func1)<<<DIV_UP(N, THREAD_N), THREAD_N>>>(dInput, dOutput, a);
    checkHIPErrors(hipMemcpy(hOutput, dOutput, sizeof(int)*N, hipMemcpyDeviceToHost));
    ...
    func2 = simple_kernel;
    memset(&attr, 0, sizeof(attr));
    checkHIPErrors(hipFuncSetCacheConfig(reinterpret_cast<const void*>(*func2), hipFuncCachePreferShared));
    checkHIPErrors(hipFuncGetAttributes(&attr, reinterpret_cast<const void*>(*func2)));
    OUTPUT_ATTR(attr);
    ...
    func3 = simple_kernel;
    memset(&attr, 0, sizeof(attr));
    checkHIPErrors(hipFuncSetCacheConfig(reinterpret_cast<const void*>(*func3), hipFuncCachePreferShared));
    checkHIPErrors(hipFuncGetAttributes(&attr, reinterpret_cast<const void*>(*func3)));
    OUTPUT_ATTR(attr);
    ...
    checkHIPErrors(hipHostFree(hOutput));
    checkHIPErrors(hipHostFree(hInput));
