...
int main(int argc, char *argv[])
{
    ...
    for (int i =0; i<STREAM_COUNT; ++i)
    {
        ...
        checkHIPErrors(hipHostMalloc(&h_data_in[i], memsize,
                                      hipHostMallocDefault));
        checkHIPErrors(hipMalloc(&d_data_in[i], memsize));
        checkHIPErrors(hipMemset(d_data_in[i], 0, memsize));

        checkHIPErrors(hipHostMalloc(&h_data_out[i], memsize,
                                      hipHostMallocDefault));
        checkHIPErrors(hipMalloc(&d_data_out[i], memsize));

        checkHIPErrors(hipStreamCreate(&stream[i]));
        checkHIPErrors(hipEventCreate(&cycleDone[i]));
    ...
    checkHIPErrors(hipMemcpyAsync(d_data_in[0], h_data_in[0], memsize,
    ...
    hipEventRecord(start,0);
    checkHIPErrors(hipMemcpyAsync(h_data_out[0], d_data_out[0], memsize,
