...
runTest(int argc, char **argv, int len)
{
    ...
    checkHIPErrors(hipMemcpy(d_idata, h_idata, mem_size,
    ...
    // execute the kernel
    hipLaunchKernelGGL(HIP_KERNEL_NAME(testKernel<T>), dim3(grid), dim3(threads), mem_size , 0, d_idata, d_odata);
    ...
    checkHIPErrors(hipMemcpy(h_odata, d_odata, sizeof(T) * num_threads,
