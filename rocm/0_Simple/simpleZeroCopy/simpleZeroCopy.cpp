...
int main(int argc, char **argv)
{
    ...
    checkHIPErrors(hipSetDeviceFlags(hipDeviceMapHost));
#else
    ...
    if (bPinGenericMemory)
    {
        ...
        checkHIPErrors(hipHostRegister(a, bytes, hipHostRegisterMapped));
        checkHIPErrors(hipHostRegister(b, bytes, hipHostRegisterMapped));
        checkHIPErrors(hipHostRegister(c, bytes, hipHostRegisterMapped));
#endif
    }
    else
    {
        ...
        checkHIPErrors(hipHostMalloc((void **)&a, bytes, flags));
        checkHIPErrors(hipHostMalloc((void **)&b, bytes, flags));
        checkHIPErrors(hipHostMalloc((void **)&c, bytes, flags));
#endif
    }
...
#if CUDART_VERSION >= 2020
    checkHIPErrors(hipHostGetDevicePointer((void **)&d_a, (void *)a, 0));
    checkHIPErrors(hipHostGetDevicePointer((void **)&d_b, (void *)b, 0));
    checkHIPErrors(hipHostGetDevicePointer((void **)&d_c, (void *)c, 0));
#endif
    ...
    getLastHIPError("vectorAddGPU() execution failed");
    ...
    {
#if CUDART_VERSION >= 4000
        checkHIPErrors(hipHostUnregister(a));
        checkHIPErrors(hipHostUnregister(b));
        checkHIPErrors(hipHostUnregister(c));
        free(a_UA);
        free(b_UA);
        free(c_UA);
#endif
    ...
    else
    {
        ...
        checkHIPErrors(hipHostFree(b));
        checkHIPErrors(hipHostFree(c));
#endif
    }
