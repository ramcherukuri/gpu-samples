...
int main(int argc, char *argv[])
{
    ...
    #pragma omp parallel
    {
        ...
        int gpu_id = -1;
        checkHIPErrors(hipSetDevice(cpu_thread_id % num_gpus));   // "% num_gpus" allows more CPU threads than GPU devices
        checkHIPErrors(hipGetDevice(&gpu_id));
        ...
        checkHIPErrors(hipMalloc((void **)&d_a, nbytes_per_kernel));
        checkHIPErrors(hipMemset(d_a, 0, nbytes_per_kernel));
        checkHIPErrors(hipMemcpy(d_a, sub_a, nbytes_per_kernel, hipMemcpyHostToDevice));
        hipLaunchKernelGGL(kernelAddConstant, dim3(gpu_blocks), dim3(gpu_threads), 0, 0, d_a, b);

        checkHIPErrors(hipMemcpy(sub_a, d_a, nbytes_per_kernel, hipMemcpyDeviceToHost));
