...
int main(int argc, char *argv[])
{
    ...
    checkHIPErrors(hipGetDeviceProperties(&devProp, devID));
    ...
    for (int i = 0; i < 2; ++i)
    {
        checkHIPErrors(hipHostMalloc((void**)&vec[i], size*sizeof*vec[i]));
        checkHIPErrors(hipMalloc((void**)&devVec[i], size*sizeof*devVec[i]));
    }

    checkHIPErrors(hipHostMalloc((void**)&results, NUM_OF_BLOCKS*sizeof*results));
    checkHIPErrors(hipMalloc((void**)&devResults, NUM_OF_BLOCKS*sizeof*devResults));

    for (int i = 0; i < 2; ++i)
    {
        generateInput(vec[i], size);
        checkHIPErrors(hipMemcpy(devVec[i], vec[i], size*sizeof*vec[i], hipMemcpyHostToDevice));
    }

    hipLaunchKernelGGL(scalarProductKernel_native, dim3(NUM_OF_BLOCKS), dim3(NUM_OF_THREADS), 0, 0, devVec[0], devVec[1], devResults, size);

    checkHIPErrors(hipMemcpy(results, devResults, NUM_OF_BLOCKS*sizeof*results, hipMemcpyDeviceToHost));
    ...
    hipLaunchKernelGGL(scalarProductKernel_intrinsics, dim3(NUM_OF_BLOCKS), dim3(NUM_OF_THREADS), 0, 0, devVec[0], devVec[1], devResults, size);
    ...
    for (int i = 0; i < 2; ++i)
    {
        checkHIPErrors(hipFree(devVec[i]));
        checkHIPErrors(hipHostFree(vec[i]));
    }

    checkHIPErrors(hipFree(devResults));
    checkHIPErrors(hipHostFree(results));
