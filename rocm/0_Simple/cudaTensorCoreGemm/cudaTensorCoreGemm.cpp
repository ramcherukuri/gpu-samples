...
int main(int argc, char **argv)
{
    ...
    checkHIPErrors(hipMalloc((void**)&A, sizeof(half) * M_GLOBAL * K_GLOBAL));
    checkHIPErrors(hipMalloc((void**)&B, sizeof(half) * N_GLOBAL * K_GLOBAL));
    checkHIPErrors(hipMalloc((void**)&C, sizeof(float) * M_GLOBAL * N_GLOBAL));
    checkHIPErrors(hipMalloc((void**)&D, sizeof(float) * M_GLOBAL * N_GLOBAL));
    ...
    checkHIPErrors(hipMemcpy(A, A_h, sizeof(half) * M_GLOBAL * K_GLOBAL, hipMemcpyHostToDevice));
    checkHIPErrors(hipMemcpy(B, B_h, sizeof(half) * N_GLOBAL * K_GLOBAL, hipMemcpyHostToDevice));
    checkHIPErrors(hipMemcpy(C, C_h, sizeof(float) * M_GLOBAL * N_GLOBAL, hipMemcpyHostToDevice));
    checkHIPErrors(hipMemset(D, 0, sizeof(float) * M_GLOBAL * N_GLOBAL));
    ...
    if (deviceProp.sharedMemPerMultiprocessor >= SHMEM_SZ)
    {
        ...
        checkHIPErrors(cudaFuncSetAttribute(compute_gemm, cudaFuncAttributeMaxDynamicSharedMemorySize, SHMEM_SZ));
        checkKernelErrors((hipLaunchKernelGGL(compute_gemm, dim3(deviceProp.multiProcessorCount), dim3(THREADS_PER_BLOCK), SHMEM_SZ, 0, A, B, C, D, alpha, beta)));
#if CPU_DEBUG
        checkHIPErrors(hipMemcpy(result_hD, D, sizeof(float)*M_GLOBAL*N_GLOBAL, hipMemcpyDeviceToHost));
#endif
    }
    else
    {
