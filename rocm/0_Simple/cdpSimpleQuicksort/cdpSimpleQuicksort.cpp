...
void run_qsort(unsigned int *data, unsigned int nitems)
{
    ...
    checkHIPErrors(hipDeviceSetLimit(cudaLimitDevRuntimeSyncDepth, MAX_DEPTH));

    // Launch on device
    int left = 0;
    int right = nitems-1;
    std::cout << "Launching kernel on the GPU" << std::endl;
    hipLaunchKernelGGL(cdp_simple_quicksort, dim3(1), dim3(1 ), 0, 0, data, left, right, 0);
...
void check_results(int n, unsigned int *results_d)
{
    ...
    checkHIPErrors(hipMemcpy(results_h, results_d, n*sizeof(unsigned), hipMemcpyDeviceToHost));
...
int main(int argc, char **argv)
{
    ...
    device = findHIPDevice(argc, (const char **)argv);
    checkHIPErrors(hipGetDeviceProperties(&deviceProp, device));
    ...
    // Allocate GPU memory.
    checkHIPErrors(hipMalloc((void **)&d_data, num_items * sizeof(unsigned int)));
    checkHIPErrors(hipMemcpy(d_data, h_data, num_items * sizeof(unsigned int), hipMemcpyHostToDevice));
