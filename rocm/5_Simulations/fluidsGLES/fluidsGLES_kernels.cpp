...
#include <helper_hip.h>    // Helper functions for CUDA Error handling
...
void setupTexture(int x, int y)
{
    ...
    getLastHIPError("hipMalloc failed");
    ...
    checkHIPErrors(hipCreateTextureObject(&texObj, &texRes, &texDescr, NULL));
}

void updateTexture(cData *data, size_t wib, size_t h, size_t pitch)
{
    checkHIPErrors(hipMemcpy2DToArray(array, 0, 0, data, pitch, wib, h, hipMemcpyDeviceToDevice));
}
...
void addForces(cData *v, int dx, int dy, int spx, int spy, float fx, float fy, int r)
{
    ...
    getLastHIPError("addForces_k failed.");
}
...
void advectVelocity(cData *v, float *vx, float *vy, int dx, int pdx, int dy, float dt)
{
    ...
    getLastHIPError("advectVelocity_k failed.");
}
...
{
    // Forward FFT
    checkHIPErrors(hipfftExecR2C(planr2c, (hipfftReal *)vx, (hipfftComplex *)vx));
    checkHIPErrors(hipfftExecR2C(planr2c, (hipfftReal *)vy, (hipfftComplex *)vy));
    ...
    getLastHIPError("diffuseProject_k failed.");

    // Inverse FFT
    checkHIPErrors(hipfftExecC2R(planc2r, (hipfftComplex *)vx, (hipfftReal *)vx));
    checkHIPErrors(hipfftExecC2R(planc2r, (hipfftComplex *)vy, (hipfftReal *)vy));
}
...
void updateVelocity(cData *v, float *vx, float *vy, int dx, int pdx, int dy)
{
    ...
    getLastHIPError("updateVelocity_k failed.");
}
...
void advectParticles(GLuint vbo, cData *v, int dx, int dy, float dt)
{
    ...
    cData *p;
    checkHIPErrors(cudaGraphicsMapResources(1, &cuda_vbo_resource, 0));
    getLastHIPError("cudaGraphicsMapResources failed");

    size_t num_bytes;
    checkHIPErrors(cudaGraphicsResourceGetMappedPointer((void **)&p, &num_bytes,
                                         cuda_vbo_resource));
    getLastHIPError("cudaGraphicsResourceGetMappedPointer failed");

    hipLaunchKernelGGL(advectParticles_k, dim3(grid), dim3(tids), 0, 0, p, v, dx, dy, dt, TILEY/TIDSY, tPitch);
    getLastHIPError("advectParticles_k failed.");

    checkHIPErrors(cudaGraphicsUnmapResources(1, &cuda_vbo_resource, 0));
}
