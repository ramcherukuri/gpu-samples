                          ...
                          bool bUsePBO)
{
    if (bUsePBO)
    {
        checkHIPErrors(cudaGraphicsResourceSetMapFlags(pgres[currentRead], cudaGraphicsMapFlagsReadOnly));
        checkHIPErrors(cudaGraphicsResourceSetMapFlags(pgres[1-currentRead], cudaGraphicsMapFlagsWriteDiscard));
        checkHIPErrors(cudaGraphicsMapResources(2, pgres, 0));
        size_t bytes;
        checkHIPErrors(cudaGraphicsResourceGetMappedPointer((void **)&(deviceData[0].dPos[currentRead]), &bytes, pgres[currentRead]));
        checkHIPErrors(cudaGraphicsResourceGetMappedPointer((void **)&(deviceData[0].dPos[1-currentRead]), &bytes, pgres[1-currentRead]));
    }
    ...
    for (unsigned int dev = 0; dev != numDevices; dev++)
    {
        ...
        hipLaunchKernelGGL(HIP_KERNEL_NAME(integrateBodies<T>), dim3(numBlocks), dim3(blockSize), sharedMemSize , 0, (typename vec4<T>::Type *)deviceData[dev].dPos[1-currentRead],
        ...
        if (numDevices > 1)
        {
            checkHIPErrors(hipEventRecord(deviceData[dev].event));
    ...
    if (numDevices > 1)
    {
        for (unsigned int dev = 0; dev < numDevices; dev++)
        {
            checkHIPErrors(hipEventSynchronize(deviceData[dev].event));
        }
    }

    if (bUsePBO)
    {
        checkHIPErrors(cudaGraphicsUnmapResources(2, pgres, 0));
    }
}
