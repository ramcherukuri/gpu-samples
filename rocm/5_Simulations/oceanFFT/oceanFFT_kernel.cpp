                                ...
                                float patchSize)
{
    ...
    hipLaunchKernelGGL(generateSpectrumKernel, dim3(grid), dim3(block), 0, 0, d_h0, d_ht, in_width, out_width, out_height, animTime, patchSize);
}
                               ...
                               bool autoTest)
{
    ...
    if (autoTest)
    {
        hipLaunchKernelGGL(updateHeightmapKernel_y, dim3(grid), dim3(block), 0, 0, d_heightMap, d_ht, width);
    }
    else
    {
        hipLaunchKernelGGL(updateHeightmapKernel, dim3(grid), dim3(block), 0, 0, d_heightMap, d_ht, width);
    }
}
