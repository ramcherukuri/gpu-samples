...
{
    checkHIPErrors(hipMalloc(&plan->d_OptionData, sizeof(__TOptionData)*(plan->optionCount)));
    checkHIPErrors(hipMalloc(&plan->d_CallValue, sizeof(__TOptionValue)*(plan->optionCount)));
    checkHIPErrors(hipHostMalloc(&plan->h_OptionData, sizeof(__TOptionData)*(plan->optionCount)));
    //Allocate internal device memory
    checkHIPErrors(hipHostMalloc(&plan->h_CallValue, sizeof(__TOptionValue)*(plan->optionCount)));
    //Allocate states for pseudo random number generators
    checkHIPErrors(hipMalloc((void **) &plan->rngStates,
                               plan->gridSize * THREAD_N * sizeof(hiprandState)));
    checkHIPErrors(hipMemset(plan->rngStates, 0, plan->gridSize * THREAD_N * sizeof(hiprandState)));

    // place each device pathN random numbers apart on the random number sequence
    hipLaunchKernelGGL(rngSetupStates, dim3(plan->gridSize), dim3(THREAD_N), 0, 0, plan->rngStates, plan->device);
    getLastHIPError("rngSetupStates kernel failed.\n");
}
...
extern "C" void closeMonteCarloGPU(TOptionPlan *plan)
{
    ...
    checkHIPErrors(hipFree(plan->rngStates));
    checkHIPErrors(hipHostFree(plan->h_CallValue));
    checkHIPErrors(hipHostFree(plan->h_OptionData));
    checkHIPErrors(hipFree(plan->d_CallValue));
    checkHIPErrors(hipFree(plan->d_OptionData));
}
...
extern "C" void MonteCarloGPU(TOptionPlan *plan, hipStream_t stream)
{
    ...
    hipLaunchKernelGGL(MonteCarloOneBlockPerOption, dim3(plan->gridSize), dim3(THREAD_N), 0, stream, 
        plan->rngStates,
    ...
    getLastHIPError("MonteCarloOneBlockPerOption() execution failed\n");
