...
{
    checkHIPErrors(hipMemcpyToSymbol(HIP_SYMBOL(c_Table),
                        tableCPU,
...
extern "C" void quasirandomGeneratorGPU(float *d_Output, unsigned int seed, unsigned int N)
{
    ...
    hipLaunchKernelGGL(quasirandomGeneratorKernel, dim3(128), dim3(threads), 0, 0, d_Output, seed, N);
    getLastHIPError("quasirandomGeneratorKernel() execution failed.\n");
}
...
{
    hipLaunchKernelGGL(inverseCNDKernel, dim3(128), dim3(128), 0, 0, d_Output, d_Input, N);
    getLastHIPError("inverseCNDKernel() execution failed.\n");
}
