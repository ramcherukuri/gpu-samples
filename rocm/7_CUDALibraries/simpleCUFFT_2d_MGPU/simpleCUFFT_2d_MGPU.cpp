...
__global__ void solvePoisson(hipfftComplex *, hipfftComplex *, float *, int, int, int n_gpu);
...
int main(int argc, char **argv)
{
    ...
    if (result != HIPFFT_SUCCESS) { printf ("hipfftCreate failed\n"); exit (EXIT_FAILURE); }
...
void  solvePoissonEquation(cudaLibXtDesc *d_ft,cudaLibXtDesc *d_ft_k, float **k, int N, int nGPUs)
{
    ...
    for(int i=0; i < nGPUs ; i++)
    {
        ...
        hipSetDevice(device) ;
        hipLaunchKernelGGL(solvePoisson, dim3(dimGrid), dim3(dimBlock), 0, 0, (hipfftComplex*) d_ft->descriptor->data[i],
                                           (hipfftComplex*) d_ft_k->descriptor->data[i],
                                           k[i], N, i, nGPUs);
    }
    ...
    for(int i=0; i< nGPUs ; i++)
    {
        ...
        getLastHIPError("Kernel execution failed [ solvePoisson ]");
    }
...
////////////////////////////////////////////////////////////////////////////////
__global__ void solvePoisson(hipfftComplex *ft, hipfftComplex *ft_k, float *k, int N ,int gpu_id, int n_gpu)
{
