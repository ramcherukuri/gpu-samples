...
int main(int argc, char **argv)
{
    ...
    checkHIPErrors(hipGetDeviceProperties(&properties, device));
...
int runTest(int argc, char **argv)
{
    ...
    Complex *d_signal;
    checkHIPErrors(hipMalloc((void **)&d_signal, mem_size));
    // Copy host memory to device
    checkHIPErrors(hipMemcpy(d_signal, h_padded_signal, mem_size,
                               hipMemcpyHostToDevice));

    // Allocate device memory for filter kernel
    Complex *d_filter_kernel;
    checkHIPErrors(hipMalloc((void **)&d_filter_kernel, mem_size));
    ...
    checkHIPErrors(hipfftCreate(&plan));
    checkHIPErrors(hipfftCreate(&cb_plan));

    checkHIPErrors(hipfftMakePlan1d(plan, new_size, HIPFFT_C2C, 1, &work_size));
    checkHIPErrors(hipfftMakePlan1d(cb_plan, new_size, HIPFFT_C2C, 1, &work_size));
    ...
    cb_params *d_params;
    checkHIPErrors(hipMalloc((void **)&d_params, sizeof(cb_params)));

    // Copy host memory to device
    checkHIPErrors(hipMemcpy(d_params, &h_params, sizeof(cb_params),
                               hipMemcpyHostToDevice));

    // The host needs to get a copy of the device pointer to the callback
    cufftCallbackLoadC hostCopyOfCallbackPtr;

    checkHIPErrors(hipMemcpyFromSymbol(&hostCopyOfCallbackPtr, HIP_SYMBOL(myOwnCallbackPtr),
    ...
    checkHIPErrors(cufftXtSetCallback(cb_plan,
                                       (void **)&hostCopyOfCallbackPtr,
                                       CUFFT_CB_LD_COMPLEX,
                                       (void **)&d_params));
    ...
    checkHIPErrors(hipfftExecC2C(plan, (hipfftComplex *)d_signal, (hipfftComplex *)d_signal, HIPFFT_FORWARD));
    checkHIPErrors(hipfftExecC2C(plan, (hipfftComplex *)d_filter_kernel, (hipfftComplex *)d_filter_kernel, HIPFFT_FORWARD));
    ...
    checkHIPErrors(hipfftExecC2C(cb_plan, (hipfftComplex *)d_signal, (hipfftComplex *)d_signal, HIPFFT_BACKWARD));
    ...
    checkHIPErrors(hipfftDestroy(cb_plan));
    ...
    checkHIPErrors(hipFree(d_params));
