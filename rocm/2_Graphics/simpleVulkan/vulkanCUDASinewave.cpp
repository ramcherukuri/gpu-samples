...
private:
    ...
    void cudaInitVertexMem()
    {
        ...
        hipLaunchKernelGGL(sinewave_gen_kernel, dim3(grid), dim3(block), 0, streamToRun, vertices, mesh_width, mesh_height, 1.0);
        checkHIPErrors(hipStreamSynchronize(streamToRun));
    }
    ...
    void cudaVkImportVertexMem()
    {
        ...
        checkHIPErrors(cudaImportExternalMemory(&cudaExtMemVertexBuffer, &cudaExtMemHandleDesc));
        ...
        checkHIPErrors(cudaExternalMemoryGetMappedBuffer(&cudaDevVertptr, cudaExtMemVertexBuffer, &cudaExtBufferDesc));
    ...
    void cudaVkImportSemaphore()
    {
        ...
        checkHIPErrors(cudaImportExternalSemaphore(&cudaExtCudaUpdateVkVertexBufSemaphore, &externalSemaphoreHandleDesc));
...
#endif
        externalSemaphoreHandleDesc.flags = 0;
        checkHIPErrors(cudaImportExternalSemaphore(&cudaExtVkUpdateCudaVertexBufSemaphore, &externalSemaphoreHandleDesc));
    ...
    void cudaVkSemaphoreSignal(cudaExternalSemaphore_t &extSemaphore)
    {
        ...
        checkHIPErrors(cudaSignalExternalSemaphoresAsync(&extSemaphore, &extSemaphoreSignalParams, 1, streamToRun));
    }
    ...
    void cudaVkSemaphoreWait(cudaExternalSemaphore_t &extSemaphore)
    {
        ...
        checkHIPErrors(cudaWaitExternalSemaphoresAsync(&extSemaphore, &extSemaphoreWaitParams, 1, streamToRun));
    }
    ...
    void cudaUpdateVertexBuffer()
    {
        ...
        AnimTime += 0.01f;
        hipLaunchKernelGGL(sinewave_gen_kernel, dim3(grid), dim3(block), 0, streamToRun, pos, mesh_width, mesh_height, AnimTime);
    ...
    void cleanup() {
        ...
        checkHIPErrors(cudaDestroyExternalSemaphore(cudaExtCudaUpdateVkVertexBufSemaphore));
        vkDestroySemaphore(device, cudaUpdateVkVertexBufSemaphore, nullptr);
        checkHIPErrors(cudaDestroyExternalSemaphore(cudaExtVkUpdateCudaVertexBufSemaphore));
        ...
        checkHIPErrors(cudaDestroyExternalMemory(cudaExtMemVertexBuffer));
