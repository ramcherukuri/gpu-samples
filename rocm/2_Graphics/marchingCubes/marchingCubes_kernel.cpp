...
#include <stdio.h>
#include <string.h>
#include <helper_hip.h>    // includes for helper CUDA functions
...
{
    checkHIPErrors(hipMalloc((void **) d_edgeTable, 256*sizeof(uint)));
    checkHIPErrors(hipMemcpy((void *)*d_edgeTable, (void *)edgeTable, 256*sizeof(uint), hipMemcpyHostToDevice));
    hipChannelFormatDesc channelDesc = hipCreateChannelDesc(32, 0, 0, 0, hipChannelFormatKindUnsigned);
    checkHIPErrors(hipBindTexture(0, edgeTex, *d_edgeTable, channelDesc));

    checkHIPErrors(hipMalloc((void **) d_triTable, 256*16*sizeof(uint)));
    checkHIPErrors(hipMemcpy((void *)*d_triTable, (void *)triTable, 256*16*sizeof(uint), hipMemcpyHostToDevice));
    checkHIPErrors(hipBindTexture(0, triTex, *d_triTable, channelDesc));

    checkHIPErrors(hipMalloc((void **) d_numVertsTable, 256*sizeof(uint)));
    checkHIPErrors(hipMemcpy((void *)*d_numVertsTable, (void *)numVertsTable, 256*sizeof(uint), hipMemcpyHostToDevice));
    checkHIPErrors(hipBindTexture(0, numVertsTex, *d_numVertsTable, channelDesc));
}
...
void bindVolumeTexture(uchar *d_volume)
{
    ...
    checkHIPErrors(hipBindTexture(0, volumeTex, d_volume, hipCreateChannelDesc(8, 0, 0, 0, hipChannelFormatKindUnsigned)));
}
                     ...
                     float3 voxelSize, float isoValue)
{
    ...
    getLastHIPError("classifyVoxel failed");
}
...
launch_compactVoxels(dim3 grid, dim3 threads, uint *compactedVoxelArray, uint *voxelOccupied, uint *voxelOccupiedScan, uint numVoxels)
{
    ...
    getLastHIPError("compactVoxels failed");
}
                         ...
                         float3 voxelSize, float isoValue, uint activeVoxels, uint maxVerts)
{
    hipLaunchKernelGGL(generateTriangles, dim3(grid), dim3(NTHREADS), 0, 0, pos, norm,
                                          ...
                                          maxVerts);
    getLastHIPError("generateTriangles failed");
}
                          ...
                          float3 voxelSize, float isoValue, uint activeVoxels, uint maxVerts)
{
    hipLaunchKernelGGL(generateTriangles2, dim3(grid), dim3(NTHREADS), 0, 0, pos, norm,
                                           ...
                                           voxelSize, isoValue, activeVoxels,
                                           maxVerts);
    getLastHIPError("generateTriangles2 failed");
}
