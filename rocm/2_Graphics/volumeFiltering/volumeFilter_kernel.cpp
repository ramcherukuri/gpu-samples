...
d_filter_surface3d(int filterSize, float filter_offset,
                   hipExtent volumeSize, hipTextureObject_t volumeTexIn, hipSurfaceObject_t volumeTexOut)
{
...
Volume *VolumeFilter_runFilter(Volume *input, Volume *output0, Volume *output1, int iterations, int numWeights, float4 *weights, float postWeightOffset)
{
    ...
    // set weights
    checkHIPErrors(hipMemcpyToSymbol(HIP_SYMBOL(c_filterData), weights, sizeof(float4)*numWeights));

    for (int i = 0; i < iterations; i++)
    {
        hipLaunchKernelGGL(d_filter_surface3d, dim3(gridSize), dim3(blockSize), 0, 0, numWeights, postWeightOffset, size,
                                                    input->volumeTex, output0->volumeSurf);

        getLastHIPError("filter kernel failed");
