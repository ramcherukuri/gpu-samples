...
void call_reduceSinglePassMultiBlockCG(int size, int threads, int numBlocks, float *d_idata, float *d_odata)
{
    ...
    hipLaunchCooperativeKernel((void*)reduceSinglePassMultiBlockCG, dimGrid, dimBlock, kernelArgs, smemSize, NULL);
...
runTest(int argc, char **argv, int device)
{
    ...
    checkHIPErrors(hipOccupancyMaxActiveBlocksPerMultiprocessor(&numBlocksPerSm, reduceSinglePassMultiBlockCG, numThreads, numThreads*sizeof(double)));
    ...
    checkHIPErrors(hipMalloc((void **) &d_idata, bytes));
    checkHIPErrors(hipMalloc((void **) &d_odata, numBlocks*sizeof(float)));

    // copy data directly to device memory
    checkHIPErrors(hipMemcpy(d_idata, h_idata, bytes, hipMemcpyHostToDevice));
    checkHIPErrors(hipMemcpy(d_odata, h_idata, numBlocks*sizeof(float), hipMemcpyHostToDevice));
