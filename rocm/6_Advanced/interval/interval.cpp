...
int main(int argc,char *argv[])
{
    ...
    CHECKED_CALL(hipFuncSetCacheConfig(reinterpret_cast<const void*>(test_interval_newton<T>), hipFuncCachePreferL1));
    ...
    CHECKED_CALL(hipDeviceSetLimit(cudaLimitStackSize, 8192));
    ...
    {
        hipLaunchKernelGGL(HIP_KERNEL_NAME(test_interval_newton<T>), dim3(GRID_SIZE), dim3(BLOCK_SIZE), 0, 0, d_result, d_nresults, i, implementation_choice);
