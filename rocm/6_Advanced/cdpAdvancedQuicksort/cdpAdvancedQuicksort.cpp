...
float run_quicksort_cdp(unsigned *gpudata, unsigned *scratchdata, unsigned int count, hipStream_t stream)
{
    ...
    checkHIPErrors(hipMalloc((void **)&gpustack, stacksize * sizeof(qsortAtomicData)));
    checkHIPErrors(hipMemset(gpustack, 0, sizeof(qsortAtomicData)));     // Only need set first entry to 0
    ...
    checkHIPErrors(hipMalloc((void **)&ringbuf, sizeof(qsortRingbuf)));
    ...
    checkHIPErrors(hipMemcpy(ringbuf, &buf, sizeof(buf), hipMemcpyHostToDevice));


    // Timing events...
    hipEvent_t ev1, ev2;
    checkHIPErrors(hipEventCreate(&ev1));
    checkHIPErrors(hipEventCreate(&ev2));
    checkHIPErrors(hipEventRecord(ev1));
    ...
    checkHIPErrors(hipEventRecord(ev2));
    ...
    if (hipPeekAtLastError() != hipSuccess)
        ...
        checkHIPErrors(hipEventElapsedTime(&elapse, ev1, ev2));

    // Sanity check that the stack allocator is doing the right thing
    checkHIPErrors(hipMemcpy(&buf, ringbuf, sizeof(*ringbuf), hipMemcpyDeviceToHost));
    ...
    checkHIPErrors(hipFree(ringbuf));
    checkHIPErrors(hipFree(gpustack));
...
int run_qsort(unsigned int size, int seed, int debug, int loop, int verbose)
{
    ...
    checkHIPErrors(hipMalloc((void **)&gpudata, size*sizeof(unsigned)));
    checkHIPErrors(hipMalloc((void **)&scratchdata, size*sizeof(unsigned)));
    ...
    for (size=min; size<=max; size+=loop)
    {
        ...
        checkHIPErrors(hipMemcpy(gpudata, data, size*sizeof(unsigned), hipMemcpyHostToDevice));
        ...
        checkHIPErrors(hipMemcpy(data, gpudata, size*sizeof(unsigned), hipMemcpyDeviceToHost));
    ...
    checkHIPErrors(hipFree(scratchdata));
    checkHIPErrors(hipFree(gpudata));
    delete(data);
    return 0;
}
...
int main(int argc, char *argv[])
{
    ...
    int cuda_device = findHIPDevice(argc, (const char **)argv);
    hipDeviceProp_t properties;
    checkHIPErrors(hipGetDeviceProperties(&properties, cuda_device));
