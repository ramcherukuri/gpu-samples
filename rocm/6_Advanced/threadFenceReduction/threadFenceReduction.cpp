...
void shmoo(int minN, int maxN, int maxThreads, int maxBlocks)
{
    ...
    checkHIPErrors(hipMalloc((void **) &d_odata, maxNumBlocks*sizeof(float)));

    // copy data directly to device memory
    checkHIPErrors(hipMemcpy(d_idata, h_idata, bytes, hipMemcpyHostToDevice));
    checkHIPErrors(hipMemcpy(d_odata, h_idata, maxNumBlocks*sizeof(float), hipMemcpyHostToDevice));
