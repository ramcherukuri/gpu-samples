...
int main(int argc, char *argv[])
{
    ...
    checkHIPErrors(hipMalloc((void **)&d_Kernel, DATA_SIZE));
    checkHIPErrors(hipMalloc((void **)&d_Data,   DATA_SIZE));
    ...
    checkHIPErrors(hipMemset(d_Kernel, 0, DATA_SIZE));
    checkHIPErrors(hipMemcpy(d_Kernel, h_Kernel, KERNEL_SIZE, hipMemcpyHostToDevice));
    checkHIPErrors(hipMemcpy(d_Data,   h_Data,     DATA_SIZE, hipMemcpyHostToDevice));
    ...
    checkHIPErrors(hipMemcpy(h_ResultGPU, d_Data, DATA_SIZE, hipMemcpyDeviceToHost));
    ...
    checkHIPErrors(hipFree(d_Kernel));
    free(h_ResultGPU);
    free(h_ResultCPU);
    free(h_Data);
    free(h_Kernel);
