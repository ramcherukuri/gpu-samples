...
void getIdenticalGPUs(int num_of_gpus, std::set<int> &identicalGPUs) {
  ...
  while (it != identicalGPUs.end()) {
    hipDeviceProp_t deviceProp;
    checkHIPErrors(hipGetDeviceProperties(&deviceProp, *it));
...
int main(int argc, char **argv) {
  ...
  int num_of_gpus = 0;
  checkHIPErrors(hipGetDeviceCount(&num_of_gpus));
  ...
  checkHIPErrors(hipMallocManaged((void **)&I, sizeof(int) * (N + 1)));
  checkHIPErrors(hipMallocManaged((void **)&J, sizeof(int) * nz));
  checkHIPErrors(hipMallocManaged((void **)&val, sizeof(float) * nz));
  ...
  checkHIPErrors(hipMallocManaged((void **)&x, sizeof(float) * N));

  double *dot_result;
  checkHIPErrors(hipMallocManaged((void **)&dot_result, sizeof(double)));

  checkHIPErrors(hipMemset(dot_result, 0.0, sizeof(double)));

  // temp memory for ConjugateGradient
  checkHIPErrors(hipMallocManaged((void **)&r, N * sizeof(float)));
  checkHIPErrors(hipMallocManaged((void **)&p, N * sizeof(float)));
  checkHIPErrors(hipMallocManaged((void **)&Ax, N * sizeof(float)));
  ...
  hipDeviceProp_t deviceProp;
  checkHIPErrors(hipSetDevice(*deviceId));
  checkHIPErrors(hipGetDeviceProperties(&deviceProp, *deviceId));
  ...
  while (deviceId != identicalGPUs.end()) {
    ...
    checkHIPErrors(hipSetDevice(*deviceId));
    checkHIPErrors(hipGetDeviceProperties(&deviceProp, *deviceId));
    checkHIPErrors(hipStreamCreate(&nStreams[device_count]));
    ...
    if (deviceProp.concurrentManagedAccess) {
      ...
      checkHIPErrors(cudaMemPrefetchAsync(I, sizeof(int) * N, *deviceId,
                                           nStreams[device_count]));
      checkHIPErrors(cudaMemPrefetchAsync(val, sizeof(float) * nz, *deviceId,
                                           nStreams[device_count]));
      checkHIPErrors(cudaMemPrefetchAsync(J, sizeof(float) * nz, *deviceId,
                                           nStreams[device_count]));
  ...
  checkHIPErrors(hipLaunchCooperativeKernelMultiDevice(
  ...
  while (deviceId != identicalGPUs.end()) {
    checkHIPErrors(hipSetDevice(*deviceId));
    checkHIPErrors(hipStreamSynchronize(nStreams[device_count++]));
    deviceId++;
  }
  ...
  checkHIPErrors(hipFree(I));
  checkHIPErrors(hipFree(J));
  checkHIPErrors(hipFree(val));
  checkHIPErrors(hipFree(x));
  checkHIPErrors(hipFree(r));
  checkHIPErrors(hipFree(p));
  checkHIPErrors(hipFree(Ax));
  checkHIPErrors(hipFree(dot_result));
  free(val_cpu);
