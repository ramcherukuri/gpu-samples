...
)
{
    ...
    if (sortDir)
    {
        hipLaunchKernelGGL(HIP_KERNEL_NAME(mergeSortSharedKernel<1U>), dim3(blockCount), dim3(threadCount), 0, 0, d_DstKey, d_DstVal, d_SrcKey, d_SrcVal, arrayLength);
        getLastHIPError("mergeSortShared<1><<<>>> failed\n");
    }
    else
    {
        hipLaunchKernelGGL(HIP_KERNEL_NAME(mergeSortSharedKernel<0U>), dim3(blockCount), dim3(threadCount), 0, 0, d_DstKey, d_DstVal, d_SrcKey, d_SrcVal, arrayLength);
...
)
{
    ...
    if (sortDir)
    {
        hipLaunchKernelGGL(HIP_KERNEL_NAME(generateSampleRanksKernel<1U>), dim3(iDivUp(threadCount), dim3(256)), 256, 0, d_RanksA, d_RanksB, d_SrcKey, stride, N, threadCount);
        getLastHIPError("generateSampleRanksKernel<1U><<<>>> failed\n");
    }
    else
    {
        hipLaunchKernelGGL(HIP_KERNEL_NAME(generateSampleRanksKernel<0U>), dim3(iDivUp(threadCount), dim3(256)), 256, 0, d_RanksA, d_RanksB, d_SrcKey, stride, N, threadCount);
...
)
{
    ...
    hipLaunchKernelGGL(mergeRanksAndIndicesKernel, dim3(iDivUp(threadCount), dim3(256)), 256, 0, 
        d_LimitsA,
        d_RanksA,
        stride,
        N,
...
)
{
    ...
    if (sortDir)
    {
        hipLaunchKernelGGL(HIP_KERNEL_NAME(mergeElementaryIntervalsKernel<1U>), dim3(mergePairs), dim3(SAMPLE_STRIDE), 0, 0, 
            d_DstKey,
            d_DstVal,
            d_SrcKey,
            d_SrcVal,
        ...
        getLastHIPError("mergeElementaryIntervalsKernel<1> failed\n");
    }
    else
    {
        hipLaunchKernelGGL(HIP_KERNEL_NAME(mergeElementaryIntervalsKernel<0U>), dim3(mergePairs), dim3(SAMPLE_STRIDE), 0, 0, 
            d_DstKey,
            d_DstVal,
            d_SrcKey,
            d_SrcVal,
        ...
        getLastHIPError("mergeElementaryIntervalsKernel<0> failed\n");
    }
}
...
{
    checkHIPErrors(hipMalloc((void **)&d_RanksA,  MAX_SAMPLE_COUNT * sizeof(uint)));
    checkHIPErrors(hipMalloc((void **)&d_RanksB,  MAX_SAMPLE_COUNT * sizeof(uint)));
    checkHIPErrors(hipMalloc((void **)&d_LimitsA, MAX_SAMPLE_COUNT * sizeof(uint)));
    checkHIPErrors(hipMalloc((void **)&d_LimitsB, MAX_SAMPLE_COUNT * sizeof(uint)));
}

extern "C" void closeMergeSort(void)
{
    checkHIPErrors(hipFree(d_RanksA));
    checkHIPErrors(hipFree(d_RanksB));
    checkHIPErrors(hipFree(d_LimitsB));
    checkHIPErrors(hipFree(d_LimitsA));
}
...
)
{
    ...
    for (uint stride = SHARED_SIZE_LIMIT; stride < N; stride <<= 1)
    {
        ...
        if (lastSegmentElements <= stride)
        {
            ...
            checkHIPErrors(hipMemcpy(okey + (N - lastSegmentElements), ikey + (N - lastSegmentElements), lastSegmentElements * sizeof(uint), hipMemcpyDeviceToDevice));
            checkHIPErrors(hipMemcpy(oval + (N - lastSegmentElements), ival + (N - lastSegmentElements), lastSegmentElements * sizeof(uint), hipMemcpyDeviceToDevice));
        }
