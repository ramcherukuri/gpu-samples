...
HIP_DYNAMIC_SHARED( unsigned char, LocalBlock)
...
SobelTex(Pixel *pSobelOriginal, unsigned int Pitch,
         int w, int h, float fScale, int blockOperation, pointFunction_t pPointOperation, hipTextureObject_t tex)
{
    unsigned char *pSobel =
        (unsigned char *)(((char *) pSobelOriginal)+blockIdx.x*Pitch);
    unsigned char tmp = 0;
...
extern "C" void setupTexture(int iw, int ih, Pixel *data, int Bpp)
{
    ...
    checkHIPErrors(hipMallocArray(&array, &desc, iw, ih));
    checkHIPErrors(hipMemcpy2DToArray(array, 0, 0, data, iw * Bpp * sizeof(Pixel), 
...
void setupFunctionTables()
{
    ...
    checkHIPErrors(hipMemcpyFromSymbol(&h_blockFunction_table[SOBEL_FILTER], HIP_SYMBOL(pComputeSobel), sizeof(blockFunction_t)));
    checkHIPErrors(hipMemcpyFromSymbol(&h_blockFunction_table[BOX_FILTER], HIP_SYMBOL(pComputeBox), sizeof(blockFunction_t)));

    // do the same for the point function, where the 2nd function is NULL ("no-op" filter, skipped in kernel code)
    checkHIPErrors(hipMemcpyFromSymbol(&h_pointFunction_table[THRESHOLD_FILTER], HIP_SYMBOL(pComputeThreshold), sizeof(pointFunction_t)));
    ...
    checkHIPErrors(hipMemcpyToSymbol(HIP_SYMBOL(blockFunction_table), h_blockFunction_table, sizeof(blockFunction_t)*LAST_BLOCK_FILTER));
    checkHIPErrors(hipMemcpyToSymbol(HIP_SYMBOL(pointFunction_table), h_pointFunction_table, sizeof(pointFunction_t)*LAST_POINT_FILTER));
}
...
extern "C" void sobelFilter(Pixel *odata, int iw, int ih, enum SobelDisplayMode mode, float fScale, int blockOperation, int pointOperation)
{
    ...
    switch (mode)
    {
        case SOBELDISPLAY_IMAGE:
            hipLaunchKernelGGL(SobelCopyImage, dim3(ih), dim3(384), 0, 0, odata, iw, iw, ih, fScale, tex);
            break;

        case SOBELDISPLAY_SOBELTEX:
            hipLaunchKernelGGL(SobelTex, dim3(ih), dim3(384), 0, 0, odata, iw, iw, ih, fScale, blockOperation, pPointOp, tex);
            break;
