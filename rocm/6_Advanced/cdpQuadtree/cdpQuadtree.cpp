...
#pragma unroll
    ...
    if (!(params.depth >= params.max_depth || num_points <= params.min_points_per_node))
    {
        ...
        if (threadIdx.x == NUM_THREADS_PER_BLOCK-1 )
        {
            ...
            // Launch 4 children.
            hipLaunchKernelGGL(HIP_KERNEL_NAME(build_quadtree_kernel<NUM_THREADS_PER_BLOCK>), dim3(4), dim3(NUM_THREADS_PER_BLOCK), 4 *NUM_WARPS_PER_BLOCK *sizeof(int), 0, children, points, Parameters(params, true));
        }
    }
}
...
bool cdpQuadtree(int warp_size)
{
    ...
    Points *points;
    checkHIPErrors(hipMalloc((void **) &points, 2*sizeof(Points)));
    checkHIPErrors(hipMemcpy(points, points_init, 2*sizeof(Points), hipMemcpyHostToDevice));
    ...
    Quadtree_node *nodes;
    checkHIPErrors(hipMalloc((void **) &nodes, max_nodes*sizeof(Quadtree_node)));
    checkHIPErrors(hipMemcpy(nodes, &root, sizeof(Quadtree_node), hipMemcpyHostToDevice));
    ...
    hipLaunchKernelGGL(HIP_KERNEL_NAME(build_quadtree_kernel<NUM_THREADS_PER_BLOCK>), dim3(1), dim3(NUM_THREADS_PER_BLOCK), smem_size, 0, nodes, points, params);
    ...
    checkHIPErrors(hipMemcpy(host_nodes, nodes, max_nodes *sizeof(Quadtree_node), hipMemcpyDeviceToHost));
    ...
    // Free memory.
    checkHIPErrors(hipFree(nodes));
    checkHIPErrors(hipFree(points));
...
int main(int argc, char **argv)
{
    ...
    checkHIPErrors(hipGetDeviceProperties(&deviceProps, cuda_device));
