...
)
{
    ...
    if (arrayLength <= SHARED_SIZE_LIMIT)
    {
        ...
        hipLaunchKernelGGL(oddEvenMergeSortShared, dim3(blockCount), dim3(threadCount), 0, 0, d_DstKey, d_DstVal, d_SrcKey, d_SrcVal, arrayLength, dir);
    }
    else
    {
        hipLaunchKernelGGL(oddEvenMergeSortShared, dim3(blockCount), dim3(threadCount), 0, 0, d_DstKey, d_DstVal, d_SrcKey, d_SrcVal, SHARED_SIZE_LIMIT, dir);
        ...
        for (uint size = 2 * SHARED_SIZE_LIMIT; size <= arrayLength; size <<= 1)
            for (unsigned stride = size / 2; stride > 0; stride >>= 1)
            {
                ...
                hipLaunchKernelGGL(oddEvenMergeGlobal, dim3((batchSize * arrayLength) / 512), dim3(256), 0, 0, d_DstKey, d_DstVal, d_DstKey, d_DstVal, arrayLength, size, stride, dir);
            }
    }
}
