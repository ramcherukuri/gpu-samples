...
initResultSmallMatrix(ResultDataSmall &result, const unsigned int mat_size)
{
    ...
    checkHIPErrors(hipMalloc((void **) &result.g_left, result.mat_size_f));
    checkHIPErrors(hipMalloc((void **) &result.g_right, result.mat_size_f));

    checkHIPErrors(hipMalloc((void **) &result.g_left_count,
                               result.mat_size_ui));
    checkHIPErrors(hipMalloc((void **) &result.g_right_count,
                               result.mat_size_ui));

    // initialize result memory
    checkHIPErrors(hipMemcpy(result.g_left, result.zero_f, result.mat_size_f,
                               hipMemcpyHostToDevice));
    checkHIPErrors(hipMemcpy(result.g_right, result.zero_f, result.mat_size_f,
                               hipMemcpyHostToDevice));
    checkHIPErrors(hipMemcpy(result.g_right_count, result.zero_ui,
                               result.mat_size_ui,
                               hipMemcpyHostToDevice));
    checkHIPErrors(hipMemcpy(result.g_left_count, result.zero_ui,
                               result.mat_size_ui,
                               hipMemcpyHostToDevice));
}
...
cleanupResultSmallMatrix(ResultDataSmall &result)
{
    ...
    checkHIPErrors(hipFree(result.g_left));
    checkHIPErrors(hipFree(result.g_right));
    checkHIPErrors(hipFree(result.g_left_count));
    checkHIPErrors(hipFree(result.g_right_count));
}
                         ...
                         const char *filename)
{
    ...
    checkHIPErrors(hipMemcpy(left, result.g_left, mat_size_f,
                               hipMemcpyDeviceToHost));
    checkHIPErrors(hipMemcpy(left_count, result.g_left_count, mat_size_ui,
                               hipMemcpyDeviceToHost));
