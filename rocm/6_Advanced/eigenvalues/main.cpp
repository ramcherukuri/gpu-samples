              ...
              const unsigned int mat_size, const unsigned int user_defined)
{
    ...
    checkHIPErrors(hipMalloc((void **) &(input.g_a)    , sizeof(float) * mat_size));
    checkHIPErrors(hipMalloc((void **) &(input.g_b_raw), sizeof(float) * mat_size));

    // copy data to device
    checkHIPErrors(hipMemcpy(input.g_a    , input.a, sizeof(float) * mat_size, hipMemcpyHostToDevice));
    checkHIPErrors(hipMemcpy(input.g_b_raw, input.b, sizeof(float) * mat_size, hipMemcpyHostToDevice));
...
cleanupInputData(InputData &input)
{
    ...
    checkHIPErrors(hipFree(input.g_a));
    input.g_a = NULL;
    checkHIPErrors(hipFree(input.g_b_raw));
    input.g_b_raw = NULL;
    input.g_b = NULL;
}
