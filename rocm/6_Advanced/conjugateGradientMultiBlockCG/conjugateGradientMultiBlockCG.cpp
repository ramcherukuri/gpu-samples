...
int main(int argc, char **argv)
{
    ...
    *dot_result = 0.0;
    ...
    checkHIPErrors(hipOccupancyMaxActiveBlocksPerMultiprocessor(&numBlocksPerSm, gpuConjugateGradient, numThreads, sMemSize));

    int numSms = deviceProp.multiProcessorCount;
    dim3 dimGrid(numSms*numBlocksPerSm, 1, 1), dimBlock(THREADS_PER_BLOCK, 1, 1);
    checkHIPErrors(hipEventRecord(start, 0));
    checkHIPErrors(hipLaunchCooperativeKernel((void *)gpuConjugateGradient, dimGrid, dimBlock, kernelArgs, sMemSize, NULL));
    checkHIPErrors(hipEventRecord(stop, 0));
    checkHIPErrors(hipDeviceSynchronize());

    float time;
    checkHIPErrors(hipEventElapsedTime(&time, start, stop));
