...
int runTest(int argc, char **argv)
{
    ...
    checkHIPErrors(hipMallocArray(&heightFieldArray, &channelDesc, dim.x, dim.y));

    // Initialize device memory
    checkHIPErrors(hipMemcpy2DToArray(heightFieldArray, 0, 0, heightField.height,
    ...
    checkHIPErrors(hipCreateTextureObject(&heightFieldTex, &texRes, &texDescr, NULL));
    ...
    // Cleanup memory
    checkHIPErrors(hipFreeArray(heightFieldArray));
    return res;
}
