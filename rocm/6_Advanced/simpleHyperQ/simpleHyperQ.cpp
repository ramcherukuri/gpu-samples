...
int main(int argc, char **argv)
{
    ...
    clock_t *a = 0;
    checkHIPErrors(hipHostMalloc((void **)&a, sizeof(clock_t)));

    // Allocate device memory for the output (one value for each kernel)
    clock_t *d_a = 0;
    checkHIPErrors(hipMalloc((void **)&d_a, 2 * nstreams * sizeof(clock_t)));
    ...
    checkHIPErrors(hipMemcpy(a, d_a, sizeof(clock_t), hipMemcpyDeviceToHost));
