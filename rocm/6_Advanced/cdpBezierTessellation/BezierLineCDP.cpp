...
__global__ void computeBezierLinesCDP(BezierLine *bLines, int nLines)
{
    ...
    if (lidx < nLines)
    {
        ...
        hipLaunchKernelGGL(computeBezierLinePositions, dim3(ceil((float)bLines[lidx].nVertices/32.0f)), dim3(32), 0, 0, lidx, bLines, bLines[lidx].nVertices);
    }
}
...
unsigned int checkCapableSM35Device(int argc, char** argv)
{
    ...
    else
    {
        ...
        {
            checkHIPErrors(hipGetDeviceProperties(&properties, i));
...
int main(int argc, char **argv)
{
    ...
    BezierLine *bLines_d;
    checkHIPErrors(hipMalloc((void **)&bLines_d, N_LINES*sizeof(BezierLine)));
    checkHIPErrors(hipMemcpy(bLines_d, bLines_h, N_LINES*sizeof(BezierLine), hipMemcpyHostToDevice));
    printf("Computing Bezier Lines (CUDA Dynamic Parallelism Version) ... ");
    hipLaunchKernelGGL(computeBezierLinesCDP, dim3((unsigned int)ceil((float)N_LINES/(float)BLOCK_DIM)), dim3(BLOCK_DIM ), 0, 0, bLines_d, N_LINES);
    printf("Done!\n");

    //Do something to draw the lines here

    hipLaunchKernelGGL(freeVertexMem, dim3((unsigned int)ceil((float)N_LINES/(float)BLOCK_DIM)), dim3(BLOCK_DIM ), 0, 0, bLines_d, N_LINES);
    checkHIPErrors(hipFree(bLines_d));
    delete[] bLines_h;
